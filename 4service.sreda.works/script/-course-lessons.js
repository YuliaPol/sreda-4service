jQuery(function ($) {
    $(document).ready(function () {
                ...
                if ($(question).children().length > 0) {
                    ...
                    QeustionElement.each(function (index, element) {
                        if ($(element).find('.course-content-name-cont').length !== 0) {
                            ...
                            $(element).find('.answer.input-shdw.lesson-name').attr('id', 'new-block_' + question_id);
                            $(element).find('.answer.input-shdw.lesson-name').attr('name', 'new-block_' + question_id);
                        }
                        else {
        ...
        //add new question
        $('.vacancy-settings').on('click', '.add-new-block', function () {
            ...
            var newQuestion =
                ...
                '            <input type="text" class="answer input-shdw lesson-name" data-reqired="reqired" value="" id="new-block_' + id_question + '" name="new-block_' + id_question + '" placeholder="Введите название блока">' +
                '       </div>' +
                '        <a href="#" class="circle-add add-answer add-new-block"' +
                ...
                '</div>' +
                '</div>';
            $('.add-new-block').data('question', id_question);
        ...
        $('.remove-popup').on('click', '.submit-remove-course', function () {
            ...
            if ($(this).data('type') == 'question') {
                ...
                var id_question = -1 + $('.add-new-block').data('question');
                $('.add-new-block').data('question', id_question);
                RefreshQuestion();
            }
        ...
        $(".vacancy-settings").on("click", ".add-new-item", function () {
            ...
            var newItem =
                ...
                '    <div class="custom-table__cell">' +
                '        <span class="table-list"><input type="text" class="answer input-shdw lesson-name" data-reqired="reqired" value="" id="new-lesson_' + id_question + '_' + id_item + '" name="new-lesson_' + id_question + '_' + id_item + '" placeholder="Введите название вопроса"></span>' +
                '    </div>' +
                '    <div class="custom-table__cell">' +
                ...
                '                   <select name="feedback-term_' + id_question + '_' + id_item + '">' +
                '                       <option value="1">1</option>' +
                '                       <option value="2">2</option>' +
                '                       <option value="3">3</option>' +
                '                       <option value="4">4</option>' +
